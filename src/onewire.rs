...
#[derive(Debug)]
pub enum Ds1820Error<E> {
    PinError(E),
    ChecksumMismatch,
    Timeout,
}

impl<E> From<E> for Ds1820Error<E> {
    fn from(error: E) -> Ds1820Error<E> {
        Ds1820Error::PinError(error)
    }
}
...
pub fn read_raw<P, E>(delay: &mut dyn Delay, pin: &mut P) -> Result<[u8; 4], Ds1820Error<E>>
where
    P: InputOutputPin<E>,
{
    pin.set_low().ok();
    ...
    if data.iter().fold(0u8, |sum, v| sum.wrapping_add(*v)) != checksum {
        Err(Ds1820Error::ChecksumMismatch)
    } else {
        Ok(data)
    }
}
